...
class DoctorsRepository {
 ...
 Future<ApiResponse<List<Doctor>>> getDoctors() async {
  ...
  return ApiResponse<List<Doctor>>.success(
    data: FakeDoctors.doctors,
  );
}
Future<ApiResponse<Doctor>> getDoctorById(String id) async {
  await Future.delayed(const Duration(milliseconds: 400));

  final doctor = FakeDoctors.doctors.firstWhere(
    (e) => e.id == id,
  );

  return ApiResponse<Doctor>.success(
    data: doctor,
  );
}
